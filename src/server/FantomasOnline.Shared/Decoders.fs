...
let optionDecoder: Decoder<FantomasOption> =
    Decode.object (fun get ->
        ...
        if t = "int" then
            get.Required.Field "$value" (Decode.tuple3 Decode.int Decode.string Decode.int) |> FantomasOption.IntOption
        else
...
let requestDecoder: Decoder<FormatRequest> =
    Decode.object (fun get ->
        { SourceCode = get.Required.Field "sourceCode" Decode.string
          Options =
              get.Required.Field "options" (Decode.list optionDecoder |> Decode.map (List.sortBy sortByOption))
