...
module GetAST =
    ...
    let private sharedChecker = lazy(FSharpChecker.Create(keepAssemblyContents = true))
    ...
    let getProjectOptionsFromScript file source defines (checker : FSharpChecker) = async {
        ...
        let! (opts, errors) = checker.GetProjectOptionsFromScript(file, source, otherFlags = otherFlags, assumeDotNetFramework = false, useSdkRefs = true)
        match errors with
        | [] ->
            return opts
    ...
    let parseAST (log:ILogger) ({ SourceCode = source; Defines = defines; IsFsi = isFsi}) =
        ...
        async {
            ...
            let! checkOptions =
                ...
                |> Async.map (fun projOpts -> checker.GetParsingOptionsFromProjectOptions projOpts |> fst)
            ...
            if untypedRes.ParseHadErrors then
                ...
                | _ ->
                    return Error Array.empty // Not sure this branch can be reached.
        }
    ...
    let private getAST log (req:HttpRequest) =
        async {
            ...
            | Result.Ok input when (input.SourceCode.Length < Const.sourceSizeLimit) ->
                ...
                | Error error ->
                    return sendInternalError (sprintf "%A" error)

            | Result.Ok _ -> // too large request
                return sendTooLargeError ()
    ...
    let private parseTypedAST (log:ILogger) ({ SourceCode = source; Defines = defines; IsFsi = isFsi}) =
            ...
            async {
                ...
                | FSharpCheckFileAnswer.Aborted ->
                    return Error (sprintf "Type checking aborted. With Parse errors:\n%A\n And with options: \n%A" parseRes.Errors options)
